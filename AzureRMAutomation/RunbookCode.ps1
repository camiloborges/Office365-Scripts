...
$currentState =	 $vms| %{
	...
	if($PowerState-eq "running"){
        $nic = $nics | ? {$vm.NetworkInterfaceIDs -contains $_.ID}
        $publicAddressID = ($nics | ? {  $vm.NetworkInterfaceIDs -contains $_.ID} | %{ $_.IPConfigurations} | Select PublicIPAddress | Select -first 1).PublicIPAddress.ID
...
###
###
### check if shutdownVMs loop can be replaced by
# $currentState | %{ $_.VM }| Stop-AzureRmVM -Force 	

#$currentState | %{ $_.VM }| Stop-AzureRmVM -Force 	
#find vms that are on for over 30 minutes that have no active  RDPSession 
$shutDownVMS = $currentState | %{
    $vm =  $_
    IF($vm.Uptime.TotalMinutes -gt $minumumLiveTime -and ($vm.RDPSessions -eq $null -or $vm.RDPSessions.Count -eq 0 )){
	    $vm
    }else{
	    $sessions =$vm.RDPSessions | ? {$_.IdleTime.TotalMinutes -gt $maximumIdleTime}
	    if($sessions -ne $null ){
		    $vm
	    }
    }
}
if($currentState.Count -ne $shutDownVMS.Count)
$shutDownVMS | %{
